...
class MongoDB:
    ...
    def saveStat(self, identifier, week, inventory, cost, out_of_stock):
        ...
        value = {'inventory': inventory, 'cost': cost, 'out_of_stock': out_of_stock}
        return self._fam(self.stat, pk, value)

    def saveOrder(self, order, week, game, player, role):
        pk = {'game': game, 'player': player, 'role': role, 'week': week, 'type': 'buy'}
        value = {'qty': order}
        return self._fam(self.order, pk, value)
    
    def saveDelivery(self, delivery, week, game, player, role):
        pk = {'game': game, 'player': player, 'role': role, 'week': week, 'type': 'delivery'}
        value = {'qty': delivery}
        return self._fam(self.order, pk, value)
    ...
    def addPlayer(self, game, player, role):
        return self.game.update_one({'name': game}, {'$set': {f'players.{player}.{role}': True}})
